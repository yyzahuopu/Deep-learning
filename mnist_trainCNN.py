...
def train(mnist):
    ...
    x = tf.placeholder(tf.float32,
            [BATCH_SIZE,
             mnist_inferenceCNN.IMAGE_SIZE,
             mnist_inferenceCNN.IMAGE_SIZE,
             mnist_inferenceCNN.NUM_CHANNELS],name='x-input')
    y_ = tf.placeholder(tf.float32,
            [None,mnist_inferenceCNN.OUTPUT_NODE],name='y-input')
    
    regularizer = tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
    
    y = mnist_inferenceCNN.inference(x,train,regularizer)
        ...
        for i in range(TRAINING_STEPS):
            ...
            reshape_xs = np.reshape(xs,
                         (BATCH_SIZE,
                          mnist_inferenceCNN.IMAGE_SIZE,
                          mnist_inferenceCNN.IMAGE_SIZE,
                          mnist_inferenceCNN.NUM_CHANNELS))
